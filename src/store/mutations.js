...
export default {
  ...
  async auth(state, mode) {
    let url =
    `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${state.firebaseWebKey}`;
    if (mode=="signup") {
      url =
      `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${state.firebaseWebKey}`;
    }
